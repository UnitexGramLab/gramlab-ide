...
class InsertLinkGrp
{
    ...
    private Hashtable idSrc, idTar; // idSrc and idTar establish
  ...
  {
    ElementImpl elt = new ElementImpl(xdoc,"xptr");
    String xptr = xp.elementAt(0).toString();
  ...
  public ElementImpl CreateLinking(DocumentImpl xdoc,Vector linking)
  {
    ...
    String ids = idTrans(linking.elementAt(1).toString());
    if(ids == null || ids.equals("")) return null;
    ElementImpl elt = new ElementImpl(xdoc,"link");
  ...
  private String idTrans(String ids)
  {
	...
	String s = (String)idSrc.get(ids.substring(x+1, y));
	if(s != null) ret += s + " ";
	x = y;
      }
      String s = (String)idSrc.get(ids.substring(x+1));
      if(s != null) ret += s;
      return ret;
    }
 ...
 public InsertLinkGrp(String fileName,
		       ...
		       Hashtable idSrc, Hashtable idTar, 
		       boolean addingMode)
  {
	...
	// un linkGrp de type alignmentXalign. Si oui, on remplace ce linkGrp
	...
	try{
	    ...
	    System.err.println("Problem while parsing "+fileName+" for writing\n");
	    System.exit(1);
	}
    }
    else{ // Ancienne forme d'appel...
	xdoc = new DocumentImpl();
	// On ajoute quand m�me un peu de sauce autour pour en faire un document TEI
	...
	whereToInsert.appendChild(nouveauxNoeuds);
	nnn = new ElementImpl(xdoc, "link");
	nnn.setAttribute("targets", srcName+" "+tarName);
    ...
    ElementImpl newDiv = new ElementImpl(xdoc, "div");
    ...
    /*ElementImpl linkGrp = new ElementImpl(xdoc,"linkGrp");
    // create XAttributeList
    linkGrp.setAttribute("domains","b1 b1");
    ...
    /*
      for (int i = 0 ; i < stcTar.size() ; i++) {
          Vector tmp = (Vector)stcTar.elementAt(i);
          linkGrp.appendChild(CreateXptr(xdoc,tmp));
          linkGrp.appendChild(new TextImpl(xdoc,"\n"));
       } 
    
    stcTar.removeAllElements(); // liberate the memory
    // put linking elements
    
    for (int i = 0 ; i < Linking.size() ; i++) {
      Vector tmp = (Vector)Linking.elementAt(i);
      ElementImpl elt = CreateLinking(xdoc, tmp);
      if(elt != null)
	linkGrp.appendChild(elt);
      linkGrp.appendChild(new TextImpl(xdoc,"\n"));
    }
    Linking.removeAllElements();
    // put link elements
    int Size = Links.size();
    for(int i=0;i<Size;i++) {
      Vector tmp = (Vector)Links.elementAt(0);
      // tmp est de la forme [d1p1s1, x1]
      // on veut l'ID de x1
      // il faut passer par stcTar
      
      ElementImpl elt = CreateConcord(xdoc, tmp);
      if(elt != null)
    ...
    /*try {
      Writer pw = new PrintWriter(new FileOutputStream(fileName));
      // Prints the XML document
      pw.write(XMLTools.STD_HEADER);
      pw.write(XMLTools.toString(xdoc));
      pw.close();
    } catch (java.io.IOException e) {
      System.err.println("InsertLinkGrp: " + e);
      e.printStackTrace();
      }*/
  }
	...
	NodeList sons = body.getChildNodes();
	Node cour;
	for (int i = 0; i < sons.getLength(); i++){
	    cour = sons.item(i);
	    if (cour.getNodeType() == Node.ELEMENT_NODE){
		if (cour.getNodeName().equals("linkGrp")){
		    if (((Element)cour).getAttribute("type").equals("alignmentXalign") || ((Element)cour).getAttribute("type").equals("essaiResultXALign")){
			body.removeChild(cour);
			// je ne fais pas de return ici des fois qu'il y en ait plusieurs :-)
		    }
		}
	    }
	}
	...
	for (int i = 0; i < sons.getLength(); i++){
		...
		if (cour.getNodeName().equals("div")){
		    if (((Element)cour).getAttribute("type").equals("resultXAlign") || ((Element)cour).getAttribute("type").equals("essaiResultXALign")){
			body.removeChild(cour);
			// je ne fais pas de return ici des fois qu'il y en ait plusieurs :-)
		    }
		}
