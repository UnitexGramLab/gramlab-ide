...
public class Sentences
{
	...
	getLengths(srcPara,tarPara); //get lengths of sentences in the src & tar
	...
	ContraintesChemin cc = cogn.cognates2Chemins(srcPara, tarPara);

	
	Path path = Align.getPath(srcLengths,tarLengths,false,cc);
	//System.out.println(path.toString());
    
	createLinks(srcPara,tarPara,path);
	createAlignementResults(srcPara, tarPara, 
				cogn.getUriSource(), cogn.getUriTarget(),
				lpt, path);
    }
  
  void getLengths(Vector srcSentences, Vector tarSentences)
  {
    int ns = srcSentences.size();
    int nt = tarSentences.size();
    ...
    for (int i = 0 ; i < ns ; i++) {
      srcLengths.setDistAt(i, ((Integer)((Vector)srcSentences.elementAt(i)).elementAt(1)).intValue());
    }
    
    for (int i = 0 ; i < nt ; i++) {
      tarLengths.setDistAt(i, ((Integer)((Vector)tarSentences.elementAt(i)).elementAt(2)).intValue());
    }
	...
	if (whereInSrc < srcId.size()){
	    currentIdSrc = (String)
		((Vector)srcId.elementAt(whereInSrc)).elementAt(0);
	...
	if (whereInTar < tarId.size()){
	    currentIdTar = (String)
		((Vector)tarId.elementAt(whereInTar)).elementAt(1);
  ...
  public void createLinks(Vector srcId,Vector Xptr, Path path)
  {
    ...
    for (int i = 0; i < path.getNumberOfPoint(); i++) {
      ...
      Vector snewLink = new Vector(), tnewLink = new Vector();

      if (pt.x > 1) {
	nbLink++;
	
	String linkName = "l"+nbLink;
	
	snewLink.addElement(linkName);
	// IDVAL: assumes no spaces in id.
	String linkIds = ((Vector)srcId.elementAt(srcCpt)).elementAt(0).toString();
	for(int j=2; j<=pt.x; j++)
	  linkIds = linkIds + " " + ((Vector)srcId.elementAt(srcCpt+j-1)).elementAt(0).toString();
	...
	String linkIds = ((Vector)Xptr.elementAt(tarCpt)).elementAt(0).toString();
	// IDVAL: assumes no space in id
	for(int j = 2 ; j <= pt.y ; j++)
	  linkIds = linkIds + " " + ((Vector)Xptr.elementAt(tarCpt+j-1)).elementAt(0).toString();
	...
	link.addElement(((Vector)srcId.elementAt(srcCpt)).elementAt(0).toString());
      else
	...
	link.addElement(((Vector) Xptr.elementAt(tarCpt)).elementAt(0).toString());
      else
