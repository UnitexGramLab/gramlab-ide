 ...
 * @version Bertrand.Gaiffe@atilf.fr :
 *          On veut prendre en entrée un fichier d'alignement qui désigne
 *          le fichier source et le fichier cible.
 */
...
public class MultiAlign {
    ...
    public static void debug(String str) {
        if (debugOut != null) debugOut.println(str);
    }
    ...
    private static void usage() {
        ...
        System.out.println("align [-d] -a sfile tfile slang tlang outfile lgfile");
        // Load, prepare and align texts
        System.out.println("Load, prepare and align texts: ");
        System.out.println("align [-d] sfile tfile src-properties tar-properties [slang tlang] lgfile");
        System.exit(0);
    }
    ...
    public static void main(String[] args) {
        ...
        if (newXA) {
            NewLoadAndPrepareTexts temp = new NewLoadAndPrepareTexts(inputAndResult);
            LoadAndPrepareTexts lpt = temp.getPreparedTexts();
            System.out.print("Aligning...");
            Div divs = new Div(lpt.divSrc, lpt.divTar,
                    lpt.paraSrc, lpt.paraTar,
                    lpt.stcSrc, lpt.stcTar, temp.getCognates(),
                    lpt);
            ...
            // Ne reste plus qu'à mettre les alignements calculés dans le fichier...
            ...
            new InsertLinkGrp(inputAndResult, // output filename.
                    lpt.lsource, lpt.ltarget, // language names
                    temp.getSourceName(),
                    temp.getTargetName(), // texts.
                    lpt.stcTar, divs.getLinking(), divs.getLinks(),
                    lpt.idSrc, lpt.idTar, true);
            ...
            while (arg < args.length) {
                ...
                if (args[arg].equals("-d")) {
                    ...
                    // java MultiAlign -p sfile tfile src-properties tar-properties outfile

                    MultiAlign.debug("" + args.length);

                    if (args.length != arg + 6) usage();
                    //
                    // Preparing for the alignement
                    //
                    new LoadAndPrepareTexts(args[arg + 1], args[arg + 2], args[arg + 3],
                            args[arg + 4], args[arg + 5]);
                    break;
                    ...
                    if (args.length != arg + 7) usage();
                    //
                    // Alignment of prepared texts
                    //
                    new AlignPreparedTexts(args[arg + 1], args[arg + 2], args[arg + 3],
                            args[arg + 4], args[arg + 5], args[arg + 6]);
                    break;
                } else if (args.length == arg + 5 || args.length == arg + 7) {
                    // java MultiAlign sfile tfile src-properties tar-properties slang tlang lgfile
                    ...
                    LoadAndPrepareTexts lpt
                            = new LoadAndPrepareTexts(args[arg], args[arg + 1],
                    ...
                    Div divs = new Div(lpt.divSrc, lpt.divTar,
                            lpt.paraSrc, lpt.paraTar,
                            lpt.stcSrc, lpt.stcTar,
                            new Cognates(args[arg], args[arg + 1]),
                            lpt);
                    ...
                    System.out.println("Writing link-group file '"
                            + args[arg + 4] + "'...");
                    new InsertLinkGrp(args[arg + 4], // output filename.
                            lpt.lsource, lpt.ltarget, // language names
                            args[arg], args[arg + 1], // texts.
                            lpt.stcTar, divs.getLinking(),
                            divs.getLinks(),
