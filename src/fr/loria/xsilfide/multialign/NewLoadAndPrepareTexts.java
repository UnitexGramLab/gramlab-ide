...
public class NewLoadAndPrepareTexts {
	...
	/* lpt = new LoadAndPrepareTexts(sourceName,
				      targetName,
	...
	public void startElement(String uri, String name, String qname,
		...
		// noCorresp
		// fuzzyAlignment
		String theType = attrs.getValue("type");
		...
		(attrs.getValue("type") != null) && 
		attrs.getValue("type").equals("alignmentDomain")){
		typeLink = attrs.getValue("type");
		if (typeLink.equals("alignmentDomain")){
		    lesFichiers = attrs.getValue("targets").trim();
		    // lesFichiers est de la forme :
		    // uri_source#id1 uri_target#id2
		    // ce qu'on veut, c'est récupérer uri_source
		    // et uri_target...
		    System.out.println("Les fichiers : "+lesFichiers);
		    filesPlusId = segmentOnWhiteSpaces(lesFichiers);
		    System.out.println("Les fichiers : \n");
		    for (Enumeration e = filesPlusId.elements();
			 e.hasMoreElements();){
	    ...
	    else if (name.equals("linkGrp") && attrs.getValue("type").equals("manualAlignment")){
		inCognates = true;
	    }
	    else if (name.equals("link") && inCognates){
		// c'est là qu'on fait la récolte des contraintes...
		// 4 types de choses :
		// des link type=linking = fabrication de paquets
		// des link type=alignment  = mise en correspondance
		// des link type=noCorresp = absent dans l'autre fichier.
		// des link type="fuzzyAlignment"
		// une difficulté à laquelle je n'avais pas pensé, on peut imposer des trucs 
		// du genre alignement de 4 phrases sur une....
		typ = attrs.getValue("type");
		if (typ.equals("linking")){
		    System.out.println("targets = "+attrs.getValue("targets")+"\n");
		    System.out.println("id = "+attrs.getValue("xml:id")+"\n");
		    System.out.println("cognates = "+cogn+"\n");
		    cogn.addPaquet(attrs.getValue("targets"),
		...
		else if (typ.equals("alignment")){
		    cogn.addAlignment(attrs.getValue("targets"), lpt);
		}
		else if (typ.equals("noCorresp")){
		    cogn.addNoCorresp(attrs.getValue("targets"), lpt);
		}
		else if (typ.equals("fuzzyAlignment")){
		    cogn.addFuzzyAlign(attrs.getValue("targets"), lpt);
		}
		else{
		...
		thePropVal = key.substring(key.lastIndexOf('.')+1,key.length()); 
	...
	public void endElement(String uri, String name, String qName){
	    ...
	    /* if (name.equals("linkGrp")){ // on ne peut pas avoir de linkGrp dans un linkGrp
		inCognates = false;
		} */
		...
		if (fileNameStatus.equals("source")){
		    ...
		    Vector tmpSrc = segmentOnChar(curFileName, '#');
		    sourceName = (String)tmpSrc.elementAt(0);
		    if (tmpSrc.size() > 1){
			idSource = (String)tmpSrc.elementAt(1);
		    }
		    ...
		    Vector tmpTar = segmentOnChar(curFileName, '#');
		    targetName = (String)tmpTar.elementAt(0);
		    if (tmpTar.size() > 1){
			idTarget = (String)tmpTar.elementAt(1);
		    }
