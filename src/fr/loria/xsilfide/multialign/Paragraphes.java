...
public class Paragraphes
{
    ...
    public Paragraphes(Vector paraSrc, Vector paraTar, Vector stceSrc, Vector stceTar, int sMax, int tMax, Cognates cogn, LoadAndPrepareTexts lpt)
  {
	...
	lgSrc += ((Integer)(((Vector)paraSrc.elementAt(i)).elementAt(1))).intValue();
      }
      
      for (int i = 0 ; i < nbTar ; i++) {
	lgTar += ((Integer)(((Vector)paraTar.elementAt(i)).elementAt(1))).intValue();
      }
	...
	while ((cSrc < nbSrc) && (tmpLgSrc < ratioSrc)) {
	  ...
	  cSrc++;
	  tmpLgSrc += ((Integer)buf.elementAt(1)).intValue();
	...
	while ((cTar < nbTar) && (tmpLgTar < ratioTar)) {
	  ...
	  cTar++;
	  tmpLgTar += ((Integer)buf.elementAt(1)).intValue();
  ...
  void getLengths(Vector paraSrc,Vector paraTar)
  {
    ...
    for (int i = 0; i < ns; i++){
      srcLengths.setDistAt(i, ((Integer)((Vector)paraSrc.elementAt(i)).elementAt(1)).intValue()/10);
    }
    
    for (int i = 0; i < nt; i++){
      tarLengths.setDistAt(i, ((Integer)((Vector)paraTar.elementAt(i)).elementAt(1)).intValue()/10);
    }
			     ...
			     
  {
	...
	if(((Integer)para.elementAt(1)).intValue()>0) {
