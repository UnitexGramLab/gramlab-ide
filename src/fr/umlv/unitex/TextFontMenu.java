 ...
 *  
 */
public class TextFontMenu extends JDialog {
  ...
  private static final int THAI_BASE = 0x0E00;
	...
	public static final int CONCORDANCE_FONT = 1;
	public static final int INPUT_FONT = 2;
	public static final int OUTPUT_FONT = 3;
	private int mode;

	/**
	 * Creates and shows a new <code>TextFontMenu</code>
	 *  
	 */
	public TextFontMenu(Preferences p,int m) {
		super(UnitexFrame.mainFrame, "Text Font", true);
		mode=m;
		pref=p;
		initFields();
		init();
    ...
    /**
     * Initializes fields with the appropriate fields of pref
     *
     */	
	private void initFields() {
		switch(mode) {
		case TEXT_FONT: {fontName = pref.textFont.getName();
		   fontStyle = pref.textFont.getStyle();
		   fontSize = pref.textFontSize;
		   break;
		}
		case CONCORDANCE_FONT: {fontName = pref.htmlFontName;
		   fontStyle = Font.PLAIN;
		   fontSize = pref.htmlFontSize;
		   break;
		}
		case INPUT_FONT: {fontName = pref.input.getName();
		   fontStyle = pref.inputFontStyle;
		   fontSize = pref.inputSize ;
		   break;
		}
		case OUTPUT_FONT: {fontName = pref.output.getName();
		   fontStyle = pref.outputFontStyle;
		   fontSize = pref.outputSize ;
		   break;
		}
		default: System.err.println("Invalid mode parameter");
		}
	}

    /**
     * Save fields with the appropriate fields of pref
     *
     */	
	void saveFields() {
		switch(mode) {
