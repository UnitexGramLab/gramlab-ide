...
public class GraphCall implements Comparable<GraphCall> {
	...
	private File grf;
	/* useful is true when the graph call is in a graph box that is
	 * both accessible and coaccessible */
	private boolean useful;
	/* direct is true when the graph is called from the main graph on which
	...
	public int compareTo(GraphCall c) {
		if (c==null) return -1;
		int n=grf.compareTo(c.getGrf());
		if (n!=0) return n;
		if (useful && !c.isUseful()) return -1;
		if (!useful && c.isUseful()) return 1;
		if (direct && !c.isDirect()) return -1;
		if (!direct && c.isDirect()) return 1;
		return 0;
	}
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof GraphCall)) return false;
		GraphCall c=(GraphCall)obj;
