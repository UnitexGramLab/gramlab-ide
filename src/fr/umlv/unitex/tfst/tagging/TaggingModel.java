...
/**
 * This class is used to know whether a sentence automaton
 * box has been hand-selected or not for tagging purpose. 
 * 
 * @author paumier
 */
public class TaggingModel {
	...
	/* We use an array that is a copy of zone's boxes, because
	 * we want to keep constant indices, even if some boxes 
	 * are removed
	 */
	TfstGraphBox[] boxes;
	...
	protected void resetModel() {
		...
		for (int i=0;i<n;i++) {
			...
			if (boxes[i].type==GenericGraphBox.INITIAL) initialState=i;
			else if (boxes[i].type==GenericGraphBox.FINAL) finalState=i;
	...
	private void computeFactorizationNodes() {
		...
		if (renumber==null) {
			/* If the automaton is not acyclic, then we
			 * fail to compute the factorization nodes, so
			 * we just say that the initial and final ones are */
			for (int i=0;i<factorization.length;i++) {
		...
		/* If we have a topological sort, then we can compute the factorization nodes */
		for (int i=0;i<factorization.length;i++) {
			factorization[i]=true;
		}
		/* We have to test if there is a transition from i to j,
		 * but with i and j relative to the sorted states */
		for (int i=0;i<boxes.length;i++) {
			int realStateI=sortedNodes[i];
			TfstGraphBox srcBox=boxes[realStateI];
			for (int j=1;j<boxes.length;j++) {
				int realStateJ=sortedNodes[j];
				TfstGraphBox destBox=boxes[realStateJ];
	...
	private int[] topologicalSort() {
		...
		int[] incoming=new int[boxes.length];
		ArrayList<Integer>[] reverse=computeReverseTransitions();
		for (int i=0;i<incoming.length;i++) {
			incoming[i]=reverse[i].size();
		}
		/* We set up the renumber array so that renumber[q] will give
		 * us the rank of the node #q after the topological sort */
		for (int q=0;q<incoming.length;q++) {
			...
			if (old==incoming.length) {
				/* If that happens, we have automaton that is not
				 * acyclic */
				sortedNodes=null;
				renumber=null;
				return null;
			...
			renumber[old]=q;
			incoming[old]=-1;
			for (GenericGraphBox gb:boxes[old].transitions) {
				int destIndex=getBoxIndex((TfstGraphBox) gb);
				incoming[destIndex]--;
			}
		}
		/* Finally, we create another array so that 
		 * sortedNodes[x] will give us the index of the node whose 
		 * rank is x */
	...
	private void markUselessStates() {
		boolean[] accessible=new boolean[boxes.length];
		boolean[] coaccessible=new boolean[boxes.length];
		...
		for (int i=0;i<accessible.length;i++) {
			if (!accessible[i] || !coaccessible[i]) {
				...
				if (taggingStates[i]==TaggingState.USELESS) {
					/* If the state used to be useless but is not anymore,
					 * we set its state to TO_BE_REMOVED */
	...
	private void checkAccessibility(int current,boolean[] visited) {
		...
		for (GenericGraphBox gb:boxes[current].transitions) {
			int destIndex=getBoxIndex((TfstGraphBox) gb);
			checkAccessibility(destIndex,visited);
		}
	}

	private void checkCoaccessibility(int current,boolean[] visited,ArrayList<Integer>[] reverse) {
		if (visited[current]) return;
		visited[current]=true;
		for (int srcIndex:reverse[current]) {
	...
	private int getPreviousFactorizationNodeIndex(int boxIndex) {
		if (renumber==null) return initialState;
		int pos=renumber[boxIndex];
		if (pos==0) return boxIndex;
		do {
			pos--;
	...
	private int getNextFactorizationNodeIndex(int boxIndex) {
		if (renumber==null) return finalState;
		int pos=renumber[boxIndex];
		if (pos==boxes.length-1) return boxIndex;
		do {
			pos++;
		} while (!factorization[sortedNodes[pos]]);
		return sortedNodes[pos];
	...
	public void selectBox(TfstGraphBox b) {
		int n=getBoxIndex(b);
		if (n==-1) throw new IllegalStateException("Should not be selecting an unknown box");
		if (taggingStates[n]==TaggingState.USELESS) {
			/* There is no point in selecting a box that cannot be
			 * part the final remaining path */
			return;
		}
	...
	/**
	 * Once we have set n as a selected box, we look if all its outgoing transitions
	 * point to TO_BE_REMOVED states. If so, those states are recursively made neutral.
	 */
	private void contaminateFollowers(int n,int limit) {
		if (n==limit) return;
		for (GenericGraphBox gb:boxes[n].transitions) {
			int destIndex=getBoxIndex((TfstGraphBox) gb);
			if (!isToBeRemovedModelIndex(destIndex)) return;
		}	
		/* If we get here, it means that all reachable states were TO_BE_REMOVED or 
		 * USELESS. Then, we have to make all the TO_BE_REMOVED ones neutral, otherwise, 
	...
	/**
	 * If there is only one outgoing transition from a selected state,
	 * then this state must be selected as well.
	 */
	private void contaminateFollowers2(int n,int limit) {
		if (n==limit) return;
		if (boxes[n].transitions.size()==1) {
			int destIndex=getBoxIndex((TfstGraphBox) boxes[n].transitions.get(0));
	...
	/**
	 * Once we have set n as a selected box, we look if all its incoming transitions
	 * point to TO_BE_REMOVED states. If so, those states are recursively made neutral.
	 */
	private void contaminateAncestors(int n,int limit,ArrayList<Integer>[] reverse) {
		if (n==limit) return;
		for (Integer srcIndex:reverse[n]) {
			if (!isToBeRemovedModelIndex(srcIndex)) return;
		}	
	...
	/**
	 * If there is only one incoming transition from a selected state,
	 * then this state must be selected as well.
	 */
	private void contaminateAncestors2(int n,int limit,ArrayList<Integer>[] reverse) {
		if (n==limit) return;
		if (reverse[n].size()==1) {
			int srcIndex=reverse[n].get(0);
	...
	ArrayList<Integer>[] computeReverseTransitions() {
		ArrayList<Integer>[] reverse=new ArrayList[boxes.length];
		for (int i=0;i<reverse.length;i++) {
			reverse[i]=new ArrayList<Integer>();
		}
		for (int i=0;i<reverse.length;i++) {
			TfstGraphBox box=boxes[i];
			if (box==null) continue;
			/* We explore all outgoing transitions */
			for (GenericGraphBox gb:box.transitions) {
				int destIndex=getBoxIndex((TfstGraphBox) gb);
	...
	/**
	 * If we select a box that is not a factorization node,
	 * then we have to update the state of boxes on alternative
	 * paths
	 */
	private void updateAlternativePaths(int boxIndex) {
		if (isFactorizationNode(boxIndex)) return;
		int a=getPreviousFactorizationNodeIndex(boxIndex);
		int b=getNextFactorizationNodeIndex(boxIndex);
		boolean[] visited=new boolean[boxes.length];
		boolean[] toBeKept=new boolean[boxes.length];
		for (int i=0;i<visited.length;i++) {
			toBeKept[i]=true;
		}
		/* We have marked candidates, but now we have to unmark 
		 * states that can be reached from the selected box */
		update(a,b,boxIndex,toBeKept,visited);
		/* Finally, we actually set TO_BE_REMOVED status for
		 * states that remain marked */
		for (int i=0;i<visited.length;i++) {
			visited[i]=false;
		}
	...
	private void unmark(int current,int limit,boolean[] toBeKept,boolean[] visited) {
		if (visited[current] || current==limit) return;
	...
	/**
	 * current and limit are nodes and n is a selected
	 * box. current will be mark as TO_BE_REMOVED if there is no
	 * path from it n.
	 */
	private boolean update(int current,int limit,int n,boolean[] toBeKept,boolean[] visited) {
		if (current==n) {
			return true;
		}
		if (current==limit) {
		...
		TfstGraphBox b=boxes[current];
		/* We explore all outgoing transitions */
		for (GenericGraphBox gb:b.transitions) {
			int destIndex=getBoxIndex((TfstGraphBox) gb);
	...
	private void setBoxStateInternal(int boxIndex,TaggingState state) {
		...
		if (boxIndex==initialState || boxIndex==finalState) return;
	...
	public TaggingState getBoxState(TfstGraphBox b) {
		if (b.type!=GenericGraphBox.NORMAL) {
			/* The initial and final states should always be 
			 * considered as neutral states */
			return TaggingState.NEUTRAL;
		}
		int index=getBoxIndex(b);
		if (index==-1) return TaggingState.NEUTRAL;
	...
	private boolean isLinearAutomaton() {
		...
		while (current!=finalState) {
			...
			if (selectedOutgoing==-1) {
				/* By convention, we do not consider that an automaton with no path
				 * is a valid linear one */
				return false;
			}
