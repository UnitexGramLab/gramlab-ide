...
public class EditionTextArea extends JTextArea {
	...
	/**
	*  Methode to get the text aera where retrieve data 
	* ( selection, text after or before the caret  )
	* @return the text's data 
	**/
	private String getData(boolean searchUp) throws BadLocationException {

		String textData = null;
		EditionTextArea txtArea = this;
		int caretPos;
		...
		if (searchUp) {
			...
			// search data betwin the current position and the end of the document
 	...
 	* @param searchUp true if Search up mode is enabled
 	* @param modelCase true if mode case is enabled
 	* @param modelWord true if wall word only mode is selected
 	* @param modelPrefixe true if prefixe search mode is selected  
 	* @param modelSuffixe true if suffixe search mode is selected
 	* @param modelRadical true if radical search mode is selected
 	* @param key the word to find
 	* @throws BadLocationException 
 	* @throws KeyErrorException if the key if null or with 0 size
 	*/
	public void findNext(
		...
		String key)
		throws BadLocationException, KeyErrorException {
		...
		do {
			...
			if (modelWord || modelPrefixe || modelSuffixe || modelRadical) {
				...
				if (index == 0)
					...
					context =
						data.substring(index - 1, index + key.length()) + " ";
				else
					context =
						data.substring(index - 1, index + key.length() + 1);
 ...
 * @param searchUp  true if Search up mode is enabled
 * @param modelCase true if case mode is selected
 * @param modelword true if wall word only mode is selected
 * @param flexionnalForm true if flexional search mode is selected
 * @param canonicalForm true if canonical search mode is selected
 * @param key the word to find
 * @throws BadLocationException 
 * @throws KeyErrorException if the key if null or with 0 size
 */
	public void dictionaryFindNext(
		boolean searchUp,
		boolean modelCase,
		boolean modelword,
		...
		do {
			...
			if ( flexionnalForm || canonicalForm || modelword ) {
				...
				if(index == 0)
					context2 = " " + data.substring(0, index + key.length() + 2);
					else 
					context2 = data.substring(index-1, index + key.length() + 2);
				}
				...
				if (canonicalForm
					&& flexionnalForm
					&& (isaFlexionalForm(key,context) || isaCanonicalForm(key,context,context2)))
					break;
				else if (
					!canonicalForm && flexionnalForm && isaFlexionalForm(key,context))
					break;
				else if (
					canonicalForm && !flexionnalForm && isaCanonicalForm(key,context,context2))
					break;
 ...
 * @param xStart the beginning of the selection
 * @param xFinish the end of the selection 
 * @param moveUp the search direction mode ( up or down )
 */
...
* @param key the word to find
* @param data the text where find the text
 ...
 */
	private boolean isWholeWord(String key, String context) {
		...
		String separtorsRegexp = new String("[^\\d\\wçéèêëàäâïîùüûôö]");
 ...
 * @param leftRegexp
 * @param rightRegexp
 * @return true if a word ("key") is surrounded by  leftRegexp and rightRegexp
 */
	private boolean isMatchingExpression(
		String key,
		String context,
		String leftRegexp,
		String rightRegexp) {

		String expr = leftRegexp + key + rightRegexp;
 ...
 * @param key
 * @param context context of key 
 * @param rightRegexp regular expression to match
 ...
 * @param key the word to find
 * @param context the context of key
 * @return true if key is a radical
 */
	private boolean isaRadical(String key, String context) {

		String leftsepartorsRegexp = new String("[\\d\\wçéèêëàäâïîùüûôö]");
		String rightSepartorsRegexp = new String("[\\d\\wçéèêëàäâïîùüûôö]");
		return isMatchingExpression(
			key,
			context,
			leftsepartorsRegexp,
 ...
 */
	private boolean isaPrefixe(String key, String context) {
		...
		String leftsepartorsRegexp = new String("[^\\d\\wçéèêëàäâïîùüûôö]");
		String rightSepartorsRegexp = new String("[\\wçéèêëàäâïîùüûôö]");
		return isMatchingExpression(
			key,
			context,
			leftsepartorsRegexp,
 ...
 */
	private boolean isaSuffixe(String key, String context) {
		...
		String leftsepartorsRegexp = new String("[\\wçéèêëàäâïîùüûôö]");
		String rightSepartorsRegexp = new String("[^\\d\\wçéèêëàäâïîùüûôö]");
		return isMatchingExpression(
			key,
			context,
 ...
 */
	private boolean isaFlexionalForm(String key, String context) {
		String leftsepartorsRegexp = new String("\n");
		String rightSepartorsRegexp = new String(",");
		return isMatchingExpression(
			key,
			context,
			leftsepartorsRegexp,
 ...
 */
	private boolean isaCanonicalForm(String key, String context, String context2) {
		...
		return isMatchingExpression(key,context,leftsepartorsRegexp,rightSepartorsRegexp) || 
			   isMatchingExpression(key,context2,"[^\\d\\wçéèêëàäâïîùüûôö]",",\\.");
	}

/**
 * find the sentence numbered X
 * @param number the sentence's number
 * @throws BadLocationException
 * @throws KeyErrorException if an error occure while sentences counting
 * @throws TextAreaSeparatorException if there isn't any sentence separator {S} or if number > sentences number in the text
 */
	public void findSentence(int number) throws BadLocationException, KeyErrorException, TextAreaSeparatorException {
		...
		String textData = getText();
 		...
 		if (countAll(key,false,false,false)+1 < number)
			...
			"This text doesn't contain "+number+" sentences, please enter an other number."
			);
	...
	public int countAll(
		...
		while (true) {
			if ((index = textData.indexOf(key)) != -1) {
				...
				if (modelWord) {
					String context = context(key, textData, index);
 ...
 * @param key the word to find
 * @param rKey the new word
 * @param modelUp  true if Search up mode is enabled
 * @param modelCase true if case mode is selected
 * @param modelWord true if wall word only mode is selected
 * @throws TargetException is key is empty
 * @throws ReplacementTargetException is the new word is empty
 */
	public void replaceAll(
		...
		boolean modelCase,
		boolean modelWord)
		throws TargetException, ReplacementTargetException {
		int index = 0;
		int textOffset = 0;
		...
		try {
			...
			if (rKey.length() == 0) {
				throw new ReplacementTargetException("Please enter replacement word");
